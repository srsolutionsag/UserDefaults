...
class ilContainerMultiSelectInputGUI extends ilMultiSelectSearchInput2GUI
{
    ...
    protected function getValueAsJson(): string
    {
        $result = [];
        if ($this->multiple) {
            $query = "SELECT obj_id, title FROM " . usrdefObj::TABLE_NAME . " WHERE type = '" . $this->getContainerType(
            ) . "' AND " .
            ...
            while ($row = $this->database->fetchAssoc($res)) {
                ...
                if ($this->with_parent) {
                    ...
                    $title = ilObject::_lookupTitle(
                        ilObject::_lookupObjectId($this->repositoryTree->getParentId($ref_id))
                    ) . ' » ' . $title;
                }
            ...
            if ($row = $this->database->fetchAssoc($res)) {
                ...
                if ($this->with_parent) {
                    $ref_id = array_shift($allReferences);
                    $title = ilObject::_lookupTitle(
                        ilObject::_lookupObjectId(self::dic()->repositoryTree()->getParentId($ref_id))
                    ) . ' » ' . $title;
                }
                if ($this->with_members && $this->getContainerType() === 'grp') {
                    ...
                    $title = $title . ' (' . $part->getCountMembers() . '/' . ($group->getMaxMembers(
                    ) == 0 ? '-' : $group->getMaxMembers()) . ')';
                }
                $result = ["id" => $row['obj_id'], "text" => $title];
            }
        }
